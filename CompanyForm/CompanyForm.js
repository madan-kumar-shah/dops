var CompanyFormModule = angular.module("myApp.CompanyForm", []);

CompanyFormModule.controller("CompanyFormCtrl", [
  "$scope",
  "$rootScope",
  "$filter",
  "$http",
  ...
  function (
    ...
    $scope.init = function () {
      ...
      $scope.myFilter();
      $scope.cinApi = $scope.GetJSONData.AOF_CP.ENTITY_DATA.BOARD_RESOL_DATE;
      ...
      $scope.FOs_list();
      $scope.getFormFieldPerTab({ form_id: "1" });
      ...
      $scope.cibil_pdf_url = "";
      // SMART PAY
      $scope.smartPayCA = $scope.GetJSONData.AOF_CP.ENTITY_DATA.IP_FUNDING?.FUNDING_MODE?.CODE?$scope.GetJSONData.AOF_CP.ENTITY_DATA.IP_FUNDING?.FUNDING_MODE?.CODE:'';
      ...
      // $scope.stakeVar =$scope.GetJSONData.AOF_CP.ENTITY_DATA.OBJECT_TYPE[0].BO.FLOW;
      // console.log('stakeVar',$scope.stakeVar);
      $scope.setSVROptions();
      ...
      if(element){
        ...
        if ($scope.form) {
          ...
          else if($scope.form[0].form_id == "7"){
            $scope.CheckBalanceFlags(); // SMART PAY
      return true;
          }
          else if($scope.form[0].form_id == "1") {
            let promoCodeChangeCount =0;
            if(promoCodeChangeCount < 1){
              $scope.getGSTValue();
                  document.querySelectorAll('[id="GSTIN No. Fetched using PAN"]').forEach(element=> 
                    {
                      if(element){
                        promoCodeChangeCount++;
          if($scope.GetJSONData.AOF_CP.ENTITY_DATA.PANGSTLINK=="0"||$scope.GetJSONData.AOF_CP.ENTITY_DATA.PANGSTLINK==null){
            $scope.GSTIN_Nos = $scope.GetJSONData.AOF_CP.ENTITY_DATA.PANGST_ARRAY
          console.log("GSTIN_Nos",$scope.GSTIN_Nos);
          element.value ="GSTIN NOT FOUND";
          }else if($scope.GetJSONData.AOF_CP.ENTITY_DATA.PANGSTLINK=="1"){
            $scope.GSTIN_Nos = $scope.GetJSONData.AOF_CP.ENTITY_DATA.PANGST_ARRAY
          console.log("GSTIN_Nos",$scope.GSTIN_Nos);
...
else if($scope.selectedQ["queue_id"]=='11'&&$scope.form[0].form_id=='6' && ($scope.loginServiceData[0].system_role=="DVUM" || $scope.loginServiceData[0].system_role=="DVUC")){
  $scope.shouldDisableCheckbox = function(optionValue) {
    return optionValue !== 'abc';
};
    return true;
}
else if($scope.form[0].form_id=='6' && ($scope.loginServiceData[0].system_role=="DVUM" || $scope.loginServiceData[0].system_role=="DVUC")){
  $scope.shouldDisableCheckbox = function(optionValue) {
...
$scope.excludeActions = function(btn) {
        ...
        SMART_PAY_ACC_BAL_STATUS_CHECK_API: [
          {
            ...
            accountNumber:$scope.GetJSONData.AOF_CP.ENTITY_DATA.IP_FUNDING?.ACC_NUM?$scope.GetJSONData.AOF_CP.ENTITY_DATA.IP_FUNDING?.ACC_NUM:'',
            transactionAmount:$scope.GetJSONData.AOF_CP.ENTITY_DATA.IP_FUNDING?.TRANSACTION_AMOUNT?$scope.GetJSONData.AOF_CP.ENTITY_DATA.IP_FUNDING?.TRANSACTION_AMOUNT:'',
          },
        ],
      };   
    $scope.showLoader("Loading.....");
        ...
        if (res.status == true) {
          ...
          if (res.data["SMART_PAY_ACC_BAL_STATUS_CHECK_API"][0].hasOwnProperty("error")) {
            ...
            $scope.insertsmartpaybalance({
              ...
              "unclearedBalance": "Techincal Failure"});
              ...
              // $scope.insertsmartpaybalance(res.data['SMART_PAY_ACC_BAL_STATUS_CHECK_API'][0].data[0].response);           
              $scope.filtercheckbalance(res.data['SMART_PAY_ACC_BAL_STATUS_CHECK_API'][0].data[0].response);
            } else {
            $scope.hideLoader();
            $scope.availableBalance ='Technical Failure';
            $scope.holdBalance ='Technical Failure';
  ...
  $scope.filtercheckbalance = function(data){
    if (
      ...
      data.hasOwnProperty("unclearedBalance")
        ...
        $scope.insertsmartpaybalance(data);
    } else {
        $scope.availableBalance ='Technical Failure';
        $scope.holdBalance ='Technical Failure';
        $scope.unclearedBalance ='Technical Failure';
  ...
  // SMART PAY
  $scope.insertsmartpaybalance = function (json) {
    var body = {
  ...
  $scope.CheckBalanceFlags = function() {
    ...
    $scope.disableRejections = function (rejID) {
      ...
      if ($scope.form[0].form_id == "2" && $scope.statusSVR) {
        if (rejID == 'RAJ233') {
          return true;
        } else {
          return false;
      ...
      if ($scope.form[0].form_id == "20" && $scope.statusSVR) {
      ...
      if ($scope.form[0].form_id == "20" && !$scope.statusSVR) {
        ...
        // SMART PAY Dynamic Rejection disable function //SMART PAY Dynamic Rejection disable in IP Funding Tab
        if($scope.form[0].form_id != "7" ){
...
//SMART PAY Dynamic Rejection disable in IP Funding Tab
$scope.SPREJECTIONDISABLELISTIPFUNDING = function () {
  var body = {
...
$scope.SPIPFUNDINGPAYMENTGATEWAYFIELDSCall = function () {
  var body = {
  ...
  executeApi(newplatwareHeader, body, function (res) {
    ...
    $scope.setRadioButtonSVR = function () {
      ...
      if ($scope.statusSVR) {
        let value = () => {
          let array = $scope.GetJSONData.AOF_CP.ENTITY_DATA.SVR.INFRASTRUCTURE;
          let valueArray = [];
    ...
    $scope.setSVROptions = function () {
      if ($scope.GetJSONData.AOF_CP.ENTITY_DATA.hasOwnProperty('ENTITY_ADD_SELECT')) {
        if ($scope.GetJSONData.AOF_CP.ENTITY_DATA.ENTITY_ADD_SELECT.hasOwnProperty('CODE')) {
          var condition1 = ($scope.GetJSONData.AOF_CP.ENTITY_DATA.ENTITY_ADD_SELECT.CODE == '1');
          // if (!$scope.GetJSONData.AOF_CP.ENTITY_DATA.hasOwnProperty('REGISTERED_CONSENT_VALUE')) {
          // } else {
          //   var condition1 = false;
          // }

          if ($scope.GetJSONData.AOF_CP.ENTITY_DATA.hasOwnProperty('COMM_ADD_PROOF_DOC')) {
            var condition2 = ($scope.GetJSONData.AOF_CP.ENTITY_DATA.ENTITY_ADD_SELECT.CODE == '3' &&
              ($scope.GetJSONData.AOF_CP.ENTITY_DATA.COMM_ADD_PROOF_DOC.CODE == '4' ||
                $scope.GetJSONData.AOF_CP.ENTITY_DATA.COMM_ADD_PROOF_DOC.CODE == '11'));
          } else {
      ...
      if (inputElement) {
        ...
        if (action === 'save') {
          if($scope.form[0].form_id == "6"){
            $scope.update_ucic_key(inputElement,'stake');
          }else{
    ...
    $scope.update_ucic_key = function (inputElement,type) {
      ...
      var body = {
        spsetucicno: [
          {
            x_lead_id: $scope.selectedGrid.x_lead_id,
            x_ucic_no: type == 'stake' ? [{ucic_num:document.getElementsByName('UCIC Number')[0].value,din_number:$scope.GetJSONData.AOF_CP.ENTITY_DATA.OBJECT_TYPE[INDEX]?.din}]:document.getElementsByName('UCIC Number')[0].value,
            x_constitution:type == 'stake' ? 'COMPANY_STAKE': "COMPANY"
          },
        ],
      };
      $scope.showLoader("Loading.....");
    ...
    $scope.getJsonDataUCIC = function (index) {
      var body = {
        spgetjson: [
          {
            x_aof_id: $scope.selectedGrid.x_lead_id,
              ...
              document.querySelectorAll('[id="UCIC Number"]').forEach(element=> 
                {
                  if(element){
                    if($scope.form[0].form_id == "1"){
                      element.value =  NewJOSN.AOF_CP.ENTITY_DATA?.UCIC_NUM?NewJOSN.AOF_CP.ENTITY_DATA?.UCIC_NUM:$scope.GetJSONData.AOF_CP.ENTITY_DATA?.UCIC_NUM;
                    }else if($scope.form[0].form_id == "6"){
                      let ucicNum =
                      NewJOSN?.AOF_CP?.ENTITY_DATA?.OBJECT_TYPE?.[index]?.BO?.UCIC_NUM ??
                      $scope.GetJSONData?.AOF_CP?.ENTITY_DATA?.OBJECT_TYPE?.[index]?.BO?.UCIC_NUM ??
                      '';
                    element.value = ucicNum?ucicNum:'';
                      // element.value =  NewJOSN.AOF_CP.ENTITY_DATA.OBJECT_TYPE[index].BO?.UCIC_NUM?NewJOSN.AOF_CP.ENTITY_DATA.OBJECT_TYPE[index].BO?.UCIC_NUM:$scope.GetJSONData.AOF_CP.ENTITY_DATA.OBJECT_TYPE[index].BO?.UCIC_NUM;
                    }
                    }            
                  });  
                  if($scope.form[0].form_id  == "6"){ 
...
$scope.DOPSPANLINKAADHAARSTATUS = function (type) {
  ...
  executeApi(newplatwareHeader, body, function (res) {
    ...
    $scope.downloadCustomerEKYCPDF = function (TYPE,INDEX) {
        ...
        EKYC_COMP_PDF: [
          {
            ...
            flag2: $scope.GetJSONData.AOF_CP.ENTITY_DATA.OBJECT_TYPE[INDEX].SEQUENCE
          },
        ],
      };
      $scope.showLoader("Loading.....");
      executeApi(newplatwareHeader, body, function (res) {
        $scope.$apply(function () {
          $scope.hideLoader();
          if (res.status == true) {
            console.log("Req body: ", body, "**/* EKYC_COMP_PDF */**", res);
            if (res.data["EKYC_COMP_PDF"][0].hasOwnProperty("error")) {
              dmDialogueBox.alertBox({
                title: "Alert",
                message: "Oop's Something went wrong",
                actionLabel: ["Ok"],
              });
            } else if (res.data["EKYC_COMP_PDF"][0].hasOwnProperty("data")) {
              $scope.decodeCustomerPdf(res.data["EKYC_COMP_PDF"][0].data[0].response, $scope.selectedGrid.x_lead_id);
            } else {
              $scope.hideLoader();
              dmDialogueBox.alertBox({
                title: "Message",
    ...
    $scope.handlePDFDownload = function(TYPE, INDEX){
      if ($scope.rejection_raised_view()) {
     $scope.SPGETCUSTOMERPDF_DB(TYPE, INDEX);
      } else {
        $scope.downloadCustomerPDF(TYPE, INDEX);
    }
    }
    $scope.downloadCustomerPDF_case_reject_scrutiny = function () {
      $scope.Company_program_variant= $scope.GetJSONData.AOF_CP.ENTITY_DATA.PRODUCT.PROGRAM_VARIANT.VALUE
        var body = {
          SEND_PDF_COMPANY_BASE64: [
            {
              leadId: $scope.selectedGrid.x_lead_id2,
              type: "ENTITY",
              pdfType:$scope.Company_program_variant,
              flag2: "x",
              flag:"Y"
            },
          ],
      ...
      executeApi(newplatwareHeader, body, function (res) {
        $scope.$apply(function () {
          $scope.hideLoader();
          if (res.status == true) {
            console.log("Req body: ", body, "**/* SEND_PDF_COMPANY_BASE64 */**", res);
             if (res.data["SEND_PDF_COMPANY_BASE64"][0].hasOwnProperty("data")) {
              $scope.SPSETCUSTOMERPDF_DB_2(res.data["SEND_PDF_COMPANY_BASE64"][0].data[0].response);
            }
          } 
        });
      });
    ...
    $scope.downloadCustomerPDF = function (TYPE, INDEX) {
          ...
          SEND_PDF_COMPANY_BASE64: [
            {
              leadId: $scope.selectedGrid.x_lead_id2,
              type: TYPE,
              pdfType:$scope.Company_program_variant, // As there is no Program variant for stake then take same as entity
              flag:"Y",
              flag2: $scope.GetJSONData.AOF_CP.ENTITY_DATA.OBJECT_TYPE[INDEX].SEQUENCE // replaced from $scope.GetJSONData.AOF_CP.ENTITY_DATA.OBJECT_TYPE[INDEX].din
            },
          ],
        };
      } else {
      ...
      executeApi(newplatwareHeader, body, function (res) {
        $scope.$apply(function () {
          ...
          if (res.status == true) {
            console.log("Req body: ", body, "**/* SEND_PDF_COMPANY_BASE64 */**", res);
            if (res.data["SEND_PDF_COMPANY_BASE64"][0].hasOwnProperty("error")) {
              dmDialogueBox.alertBox({
                title: "Alert",
                message: "Oop's Something went wrong",
                actionLabel: ["Ok"],
              });
            } else if (res.data["SEND_PDF_COMPANY_BASE64"][0].hasOwnProperty("data")) {
              $scope.decodeCustomerPdf(res.data["SEND_PDF_COMPANY_BASE64"][0].data[0].response, $scope.selectedGrid.x_lead_id);
              if (TYPE !== 'STAKE') {
                $scope.SPSETCUSTOMERPDF_DB(res.data["SEND_PDF_COMPANY_BASE64"][0].data[0].response);
              }
            } else {
              $scope.hideLoader();
              dmDialogueBox.alertBox({
        ...
        $scope.$apply(function () {
          ...
          if (res.status == true) {
            ...
            if (res.data["SPGETCUSTOMERPDF"][0].hasOwnProperty("error")) {
             ...
             if( res.data["SPGETCUSTOMERPDF"][0].data[0].STATUS =="N"){
              $scope.downloadCustomerPDF('ENTITY', INDEX);
             }else{
    ...
    //           leadId: $scope.selectedGrid.x_lead_id,
    //           type: TYPE,
    //           flag: $scope.GetJSONData.AOF_CP.ENTITY_DATA.OBJECT_TYPE[INDEX].SEQUENCE
    //         },
    //       ],
    //     };
    //   } else {
  ...
  $scope.decodeCustomerPdf = function (base64, leadID) {
    ...
    $scope.setformElementsParamGroup = function (obj) {
      ...
      if ($scope.form[0].form_id == "13") {
        $scope.getAuditTrail();
      }
      if ($scope.form[0].form_id == "12") {
        $scope.showAuditTrail = false;
      ...
      if ($scope.form[0].form_id == "6" && $scope.setRole === "") {
        $scope.getRejection_data($scope.form[0].form_id);
        // $scope.getDocumentpertab($scope.form[0]);
      }
      if ($scope.form[0].form_id == "5") {
...
//SMART PAY Dynamic Rejection disable in IP Funding Tab
if($scope.form[0].form_id == "7"){
  $scope.SPREJECTIONDISABLELISTIPFUNDING();
  $scope.SPIPFUNDINGPAYMENTGATEWAYFIELDSCall(); //SMART PAY - PAYMENT_STATUS PAID/MULTIPLE PAID HANDLING - 21Nov25
}
            } else {
              $scope.hideLoader();
              dmDialogueBox.alertBox({
    ...
    $scope.getGSTValue = function (){
      document.querySelectorAll('[id="GSTIN Date of Registration"]').forEach(element=> 
        {
      if(element){
        let GstDate =new Date($scope.GetJSONData.AOF_CP.ENTITY_DATA.GST_RESPONSE[0].result.rgdt.split('/').reverse().join('-')).toISOString().split('T')[0];
        element.value = 
        $scope.GetJSONData.AOF_CP.ENTITY_DATA.GSTIN_DOI?$scope.GetJSONData.AOF_CP.ENTITY_DATA.GSTIN_DOI:GstDate;
      }})
      document.querySelectorAll('[id="GSTIN Nature Of Business"]').forEach(element=> 
        {
      if(element){
        element.value = $scope.GetJSONData.AOF_CP.ENTITY_DATA.GST_RESPONSE[0].result.nba 
      }})
      document.querySelectorAll('[id="GSTIN Nature Of Industry"]').forEach(element=> 
        {
      if(element){
       $scope.GSTIN_NOI_Nos= $scope.GetJSONData.AOF_CP.ENTITY_DATA.GST_RESPONSE[0].result.bzgddtls
        for(let i=0;i<$scope.GSTIN_NOI_Nos.length;i++){
          let ValueGSTIN_NOI =$scope.GetJSONData.AOF_CP.ENTITY_DATA.GST_RESPONSE[0].result.bzgddtls[i].industry;
    ...
    return value.trim();
        ...
        AO_CASE_COMPANY: [
          {
            ...
            ca_constitution_field: "COMPANY/LLP",
      ...
      executeApi(newplatwareHeader, body, function (res) {
        $scope.$apply(function () {
          $scope.hideLoader();
          if (res.status == true) {
            console.log("**/* AO_CASE_COMPANY*/**", res);
            if (res.data["AO_CASE_COMPANY"][0].hasOwnProperty("error")) {
              dmDialogueBox.alertBox({
                title: "Alert",
                message: "Oop's Something went wrong",
                actionLabel: ["Ok"],
              });
            } else if (res.data["AO_CASE_COMPANY"][0].hasOwnProperty("data")) {
              if (res.data.AO_CASE_COMPANY[0].data[0].response_type == "I") {
                var response_data = res.data.AO_CASE_COMPANY[0].data[0];
                ...
                res.data.AO_CASE_COMPANY[0].data[0].response_type == "E"
              ) {
                dmDialogueBox.alertBox({
                  title: "Message",
                  message: res.data.AO_CASE_COMPANY[0].data[0].response_message,
                  actionLabel: ["Ok"],
                });
              }
            } else {
  ...
  $scope.DMSUploadGefu = function (btn) {
   ...
   if($scope.loginServiceData[0].system_role == "ACC"){
      ...
      if ($scope.countForGefu < 1) {
        ...
        executeApi(newplatwareHeader, body, function (res) {
          ...
          $scope.$apply(function () {
            $scope.hideLoader();
            if (res.status == true) {
              console.log("**/* spgeneratecompanygefufile */**", res);
              if (
                res.data["spgeneratecompanygefufile"][0].hasOwnProperty("error")
              ) {
                $scope.hideLoader();
                dmDialogueBox.alertBox({
                  title: "Alert",
                ...
                res.data["spgeneratecompanygefufile"][0].hasOwnProperty("data")
              ) {
                $scope.result_GEFU = JSON.parse(
                  res.data.spgeneratecompanygefufile[0].data[0].v_string
                );
            ...
            if (res.data["GEFUFILEDOWNLOAD"][0].hasOwnProperty("error")) {
              ...
              if (res.data.GEFUFILEDOWNLOAD[0].data[0].response_type == "I") {
                var file = dataURLtoFile(
                  ...
                  // "GEFU file.txt"
                  "G"+$scope.GetJSONData.AOF_CP.ENTITY_DATA.ACC_NUMBER+".txt" // GEFU COMPANY
                );
              } else if (
    ...
    $scope.gotosub_tabSwitching = function (x, y, index, flag) {
      if (flag == "boTAB") {
        if(index == 2){
          $scope.show_bo = false;
          $scope.show_bo_smo = true;
          $scope.SUB_BOTAB[2].openSection = true;
    ...
    $scope.getformFields = function (obj, form, index) {
          ...
          if (res.status == true) {
            ...
            if (
              ...
              if ($scope.formElements) {
                $scope.setTAB = true;
                $scope.setDATA = false;
                if(($scope.setRole=='BR' || $scope.setRole=='BR&BO' || $scope.setRole=='BO&BR' || $scope.setRole=='BO') && $scope.GetJSONData.AOF_CP.ENTITY_DATA.OBJECT_TYPE[index].BO.ID_PRF_DOC.CODE =='1'){
                  $scope.ekycPDF = true;
                }else if($scope.setRole=='AUS' || $scope.setRole=='AUS&BR&BO' || $scope.setRole=='AUS&BO&BR' || $scope.setRole=='AUS&BO' || $scope.setRole=='AUS&BR'){
                  $scope.ekycPDF = true;
                }   else{
                  $scope.ekycPDF = false;
                }            
    ...
    $scope.getvalidateforTab = function () {
      if ($scope.form[0].form_id == 6 && $scope.setDATA == true) {
        return false;
      } else return true;
    };
