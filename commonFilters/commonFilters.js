...
String.prototype.parseDate = function (formatType) {
    ...
    switch (formatType) {
        ...
        case 'servertoobj':
            ...
            returnValue = new Date(dateArr[0], (parseInt(dateArr[1]) - 1).toString(), dateArr[2], '0', '0', '0');
            break;
        case 'serverdate':
            var dateArr = this.split('-');
            var d_month = shortMonths.indexOf(dateArr[1]) + 1;
            d_month = d_month < 10 ? '0' + d_month : d_month;
            returnValue = '20' + dateArr[2] + '-' + d_month + '-' + dateArr[0];
            break;
        case 'dd-mm-yyyy':
            var dateArr = this.split('-');
            returnValue = Number(dateArr[0]).toNDigits(2) + '-' + (shortMonths.indexOf(dateArr[1]) + 1).toNDigits(2) + '-' + '20' + dateArr[2];
            break;
    }
...
Date.prototype.parseDate = function (formatType) {
    ...
    switch (formatType ? formatType.toLowerCase() : '') {
        ...
        case 'textedmonthdate':
            returnValue = (date < 10 ? '0' + date : date) + '-' + formats['shortMonths'][Number(month)] + '-' + year % 100;
            break;
        case 'isoshortdate':
...
dmCommonApi.filter('columnConfigurator', [function ($filter) {
    return function (headingColumns) {
        var currentColumnCongigurations = [];
        var allColumns = JSON.parse(JSON.stringify(headingColumns));
        for (var i = 0, max = allColumns.length; i < max; i++) {
            ...
            if (allColumns[i]['COLUMN_DATA_TYPE'].trim().toLowerCase() === "select") {
                columnObj.columnId = allColumns[i];
                columnObj.columnName = "Select";
                columnObj.columnType = "checkbox";
                columnObj.columnSequence = i;
            } else if (allColumns[i]['COLUMN_DATA_TYPE'].trim().toLowerCase() === "btn") {
                columnObj.columnId = allColumns[i]['COLUMN_OBJ_NAME'];

                columnObj.columnName = allColumns[i]['COLUMN_LABEL'];
                //  columnObj.columnName.pop();
                // columnObj.columnName = columnObj.columnName.join(' ');
                columnObj.columnType = "button";
                columnObj.columnSequence = allColumns[i]['COLUMN_SEQ'];
            }
            else if (allColumns[i]['COLUMN_DATA_TYPE'].trim().toLowerCase() === "cb") {
                columnObj.columnId = allColumns[i]['COLUMN_OBJ_NAME'];
                columnObj.columnName = allColumns[i]['COLUMN_LABEL'];
                columnObj.columnType = "check";
                columnObj.columnSequence = allColumns[i]['COLUMN_SEQ'];
            } else {
                columnObj.columnId = allColumns[i]['COLUMN_OBJ_NAME'];
                columnObj.columnName = allColumns[i]['COLUMN_LABEL'];
        ...
        //currentColumnCongigurations = $filter('orderObjectBy')(currentColumnCongigurations,'COLUMN_SEQ')
...
dmCommonApi.filter('byProp1', [function () {
    return function (collection, keyname, value, conditional) {
        ...
        if (collection && collection.length > 0 && keyname) {
            angular.forEach(collection, function (item) {
                if (item[keyname]) {
                    item[keyname] = item[keyname].toString();
                ...
                // if(item[keyname]){
                //     item[keyname]=item[keyname].toLowerCase();
                //     item[keyname]=item[keyname].trim() ;

                // }

                item[keyname] === value.trim() ? output.push(item) : "";
...
dmCommonApi.filter('byProp2', [function () {
    return function (collection, keyname, value, conditional) {
        if (conditional) {
            return collection;
        }
...
dmCommonApi.filter('objToUpper', [function () {
    return function (collection) {
        angular.forEach(collection, function (data, index) {
            var dataObj = JSON.stringify(data);
            var newDataObj = [];
            newDataObj.push(dataObj.replace(/"([^"]+)":/g, function ($0, $1) {
                var lowerCasedKey = '"' + $1.toUpperCase() + '":';
                return lowerCasedKey;
            }));
            collection.splice(index, 1, JSON.parse(newDataObj));
        });
...
dmCommonApi.service('prepareDwnldData', [function () {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            for (var j = 0; j < headers.length; j++) {
                if (headers[j]['columnType'].toLowerCase() != 'select' ||
                    ...
                    //if(headers[j]['columnName']=="CRN" || headers[j]['columnName']=="Account Number" || headers[j]['columnName']=="Mobile Number"){
                    //  string = string + '<td>' + (data[i][headers[j]['columnId'].toLowerCase()].match(/^([0-9]{4}-[0-9]{1,2}-[0-9]{1,2})/g) ? '"' + data[i][headers[j]['columnId'].toLowerCase()] + '"' : "'"+data[i][headers[j]['columnId'].toLowerCase()]) + '</td>';
                    //     // string = string + '<td>' + (data[i][headers[j]['columnId'].toLowerCase()] ?  data[i][headers[j]['columnId'].toLowerCase()]  : '') + '</td>';
                    //}else{
                    // string = string + '<td>' + (data[i][headers[j]['columnId'].toLowerCase()].match(/^([0-9]{4}-[0-9]{1,2}-[0-9]{1,2})/g) ? '"' + data[i][headers[j]['columnId'].toLowerCase()] + '"' : data[i][headers[j]['columnId'].toLowerCase()]) + '</td>';
                    string = string + '<td>' + (data[i][headers[j]['columnId'].toLowerCase()] ? data[i][headers[j]['columnId'].toLowerCase()] : '-') + '</td>';
                    //}
    ...
    this.excelDatafor_Reports = function (headers, data) {

        var string = '<table id="tableForExc"><tbody><tr>';
        for (var i = 0; i < headers.length; i++) {

            string = string + '<th>' + headers[i]['report_column'] + '</th>';
        }
        string = string + '</tr>'
        for (var i = 0; i < data.length; i++) {
            string = string + '<tr>';
            for (var j = 0; j < headers.length; j++) {
                ...
                string = string + '<td>' + (data[i][headers[j]['column_obj_name']] ? "" + data[i][headers[j]['column_obj_name']] : '') + '</td>';
    ...
    this.csvData = function (headers, data) {
        ...
        for (var i = 0, max = headers.length; i < max; i++) {
            if (i !== headers.length - 1) {
                string = string + '"' + headers[i].report_column + '",';
            } else {
                string = string + '"' + headers[i].report_column + '"';
            }
        }
        ...
        for (var j = 0; j < data.length; j++) {
            for (var k = 0; k < headers.length; k++) {
                string = string + '"' + data[j][headers[k]['column_obj_name']] + '"';
    ...
    this.textData = function (headers, data) {
        var string = '';
        for (var j = 0; j < data.length; j++) {
            for (var k = 0; k < headers.length; k++) {
                if (headers[k]['column_obj_name'] == "X_SOURCE_OF_FUND_OTHERS") {
                    var testData = JSON.parse(data[j][headers[k]['column_obj_name']]);
                    for (i = 0; i < testData.length; i++) {
                        string = string + testData[i].VALUE + ",";
                    }
                } else {
                    string = string + data[j][headers[k]['column_obj_name']];
                }
...
dmCommonApi.directive('dmTable', ['$parse', '$location', function ($parse, $location) {
    return {
        restrict: 'E',
        templateUrl: 'commonFilters/dmTable.html',
        scope: {
            downloadBtn: '=dmDownloadBtn',
            datasource: '=dmDatasource',
            searchable: "=dmSearchable",
            showingpagination: "=dmShowingPagination",
            sortable: "=dmSortable",
            //                datacount: "=dmDatacount",
            tableConfig: '=dmTableConfig',
            ...
            themeColor: '@dmTheme',
            headerColor: '@dmColor',
            csvExcelButtonColor: '@dmCsvExcelButtonColor',
        ...
        controller: function ($scope, $filter, modifyObj, prepareDwnldData) {
            ...
            $scope.getValueScrutiny = function () {
                $scope.selectedQ = JSON.parse(sessionStorage.getItem('selectedQ'));
                const queueArray = [2, 3, 4, 10, 11, 14, 17, 18, 20];
                if ($scope.selectedQ && queueArray.includes($scope.selectedQ.queue_id)) {
                    return false;
                } else {
                    return true;
                }
            }
            $scope.getValueExplore = function () {
                $scope.selectedQ = JSON.parse(sessionStorage.getItem('selectedQ'));
                const queueArray = [2, 3, 4, 10, 11, 14, 17, 18, 20];
                if ($scope.selectedQ && queueArray.includes($scope.selectedQ.queue_id)) {
                    return true;
            ...
            $scope.showBulkAccept = function () {
                $scope.selectedQ = JSON.parse(sessionStorage.getItem('selectedQ'));
                if ($scope.selectedQ && ($scope.selectedQ.queue_id == 23 || $scope.selectedQ.queue_id == 19 ||$scope.selectedQ.queue_id == 7))
                {
                    return true;
                } else {
                    return false;
            ...
            $scope.showBulkAcceptSign = function () {
                $scope.selectedQ = JSON.parse(sessionStorage.getItem('selectedQ'));
                if ($scope.selectedQ && ($scope.selectedQ.queue_id == 100))
                {
                    return true;
                } else {
                    return false;
            ...
            $scope.setDataSource = function () {
                if ($scope.datasource && $scope.datasource.length > 0) {
                    ...
                    $scope.columns = $filter('columnConfigurator')($scope.tableConfig);
                    $scope.columns = $filter('orderObjectBy')($scope.columns, 'columnSequence');
            ...
            $scope.perTabData = function () {
                ...
                $scope.columns = $filter('columnConfigurator')($scope.tableHeading.toLowerCase());
                $scope.columns = $filter('orderObjectBy')($scope.columns, 'columnSequence');
                $scope.totalPages = Math.ceil($scope.datacount / $scope.itemsPerPage.itemShown) || 0;
            ...
            $scope.calculatePerPageData = function () {
                ...
                $scope.shownTableData =
                    $filter('pagination')($scope.particularTabData,
                        $scope.pageNumber.currentPage,
                        $scope.itemsPerPage.itemShown);
                $scope.showleadcount = $scope.shownTableData.length;
                if ($scope.case_array.length == $scope.particularTabData.length) {
                    $scope.prev_method = "calculatePerPageData";
                    $scope.isAllRowSelected[$scope.pageNumber.currentPage] = ($scope.datasource.length===$scope.particularTabData.length)?true:false;
                    $scope.selectAll();
                }
                if ($scope.case_array.length == 0) {
                    $scope.prev_method = "calculatePerPageData";
                    $scope.isAllRowSelected[$scope.pageNumber.currentPage] = false;
                    $scope.selectAll();
                }
            };
            ...
            var sortTableData = function (columnName, sortType) {
                ...
                //                    if (columnName.split(' ').indexOf('date') > -1 || columnName.indexOf('last contacted') > -1) {
                ...
                for (var i = 0; i < len; i++) {
                    ...
                    for (var j = i + 1; j < len; j++) {
                        if (columnName.split(' ').indexOf('date') > -1 || columnName.indexOf('last contacted') > -1) {
                            var d1 = dataArray[j][colName] == '' ? 0 : dataArray[j][colName].parseDate('mm-dd-yyyy');
                            var d2 = dataArray[minIdx][colName] == '' ? 0 : dataArray[minIdx][colName].parseDate('mm-dd-yyyy');
                            var d1Obj = d1 != 0 ? new Date(d1) : d1;
                            var d2Obj = d2 != 0 ? new Date(d2) : d2;
                            if (sortType) {
                                if (d1Obj < d2Obj) {
                                    minIdx = j;
                                }
                            ...
                            switch (angular.isArray(dataArray[j][colName])) {
                                case true:
                                    if (sortType) {
                                        if (dataArray[j][colName].length < dataArray[minIdx][colName].length) {
                                            minIdx = j;
                                        }
                                    } else {
                                        if (dataArray[j][colName].length > dataArray[minIdx][colName].length) {
                                    ...
                                    break;
                                case false:
                                    if (angular.isNumber(dataArray[j][colName])) {
                                        if (sortType) {
                                            if (dataArray[j][colName] < dataArray[minIdx][colName]) {
                                                minIdx = j;
                                            }
                                        } else {
                                            if (dataArray[j][colName] > dataArray[minIdx][colName]) {
                                        ...
                                        if (sortType) {
                                            if (dataArray[j][colName].toLowerCase() < dataArray[minIdx][colName].toLowerCase()) {
                                                minIdx = j;
                                            }
                                        } else {
                                            if (dataArray[j][colName].toLowerCase() > dataArray[minIdx][colName].toLowerCase()) {
            ...
            $scope.prepareReturnedData = function () {
                ...
                switch (true) {
                    case $scope.tabSeperatorKey ? true :
                        false:
            ...
            // Color on dm header start here nicky
            $scope.getColor = function (headerColor) {
                if (headerColor) {
                    var s = "#" + headerColor;
                    return (!s) ? 'black' : s;
                }
                ...
                if (data.length > 0) {
                    switch (format) {
                        ...
                        case 'csv':
                            ...
                            anchor.target.download = "data.csv";
                            var hrefvalue = prepareDwnldData.uri.csv + prepareDwnldData.base64(string);
            ...
            /* $scope.downLoadTableData = function (anchor, format, ele) {
                 //$rootScope.showLoader_button();
                 var columns = $filter('byProp')($scope.columns, 'columnType', 'text')
                 var data = $scope.datasource;
 //                if ($scope.dataUniqueKey == 'dmdataindex') {
 //                    data = modifyObj.removeKey(data, $scope.dataUniqueKey);
 //                }
                 var tableHeading = $scope.datasource[0]['heading'].toLowerCase().split('~');
                 if ($scope.removeColumnDownload) {
                     var columnRemoveLength = $scope.removeColumnDownload.length;
                     if (columnRemoveLength > 0) {
                         for (var r = 0; r < columnRemoveLength; r++) {
                             var colIndex = tableHeading.indexOf($scope.removeColumnDownload[r])
                             var removedColumn = tableHeading.splice(colIndex, 1);
                 ...
                 for (var j = 0; j < tableHeading.length; j++) {
                     if (tableHeading[j].match('_btn')) {
                         var btnIndex = j;
                         var btnHeading = tableHeading.splice(btnIndex, 1);
                         break;
                     }
                 }
 
                 if (tableHeading.indexOf('select') > -1) {
                     var heading = tableHeading.splice(0, 1);
                 }
 
                 switch (format) {
                     case 'excel':
                         var source = JSON.stringify($scope.datasource);
                         // console.log(source)
                         var sqlStatement = 'SELECT ' + tableHeading.join(',').toLowerCase() + ' INTO XLSX("data.xlsx",{headers:true}) FROM ?'
                         alasql(sqlStatement, [$scope.datasource]);
                         break;
                     case 'csv':
                         var source = JSON.stringify($scope.datasource);
                         // console.log(source)
                         var sqlStatement = 'SELECT ' + tableHeading.join(',').toLowerCase() + ' INTO CSV("data.csv",{headers:true,separator:","}) FROM ?'
                         alasql(sqlStatement, [$scope.datasource]);
                         break;
                     case 'pdf':
                         break;
            ...
            $scope.editRow = function (editablePropData, index) {
                var isEditable;
            ...
            $scope.chkForAllSelected = function (data) {
                ...
                // $scope.setBulkArray('SINGLE', $scope.particularTabData[data]);
                $scope.setBulkArray('SINGLE',$scope.particularTabData.length == 1 ? $scope.particularTabData[0]:$scope.particularTabData[data]);
                console.log('single check', selectedRows);
            };

            $scope.selectAll = function () {
                    console.log('tabsepkey', $scope.tabSeperatorKey);
                    console.log('shownTableData', $scope.shownTableData);
                    let sr = { ...$scope.selectedRows };
                    let ar = { ...$scope.isAllRowSelected };
                    console.log('selRows', sr);
                    console.log('isALLrow', ar);
                    switch (true) {
                        case $scope.tabSeperatorKey ? true :
                            false:
                        ...
                        default:
                            ...
                            angular.forEach($scope.shownTableData, function (data) {
                                // console.log('default block', $scope.selectedRows, 'currentpage', $scope.pageNumber, 'data', data, 'unique', $scope.dataUniqueKey);
                                // $scope.selectedRows[$scope.pageNumber.currentPage] = $scope.selectedRows[$scope.pageNumber.currentPage] || {};
                                $scope.selectedRows[data[$scope.dataUniqueKey]] = $scope.isAllRowSelected[$scope.pageNumber.currentPage];
                            });
                            if ($scope.prev_method == 'calculatePerPageData') {
                                $scope.prev_method = "";
                            } else {
                                $scope.setBulkArray('ALL');
                            }
                            break;
                    }            
            };
            ...
            $scope.setBulkArray = function (flag, data) {
                if (flag == 'ALL') {
                    if ($scope.case_array.length == $scope.particularTabData.length) {
                        $scope.case_array.splice(0);
                    } else {
                        $scope.case_array = [...$scope.particularTabData];
                    }
                }
                if (flag == 'SINGLE') {
                    let index = $scope.case_array.findIndex(x => x.x_lead_id == data.x_lead_id);
                    if (index > -1) {
                        $scope.case_array.splice(index, 1);
                    } else {
                        $scope.case_array.push(data);
                    }
                }
                console.log('current status', $scope.case_array);
            };

            $scope.setprc_bulk = function () {
                $scope.$parent['generateGEFU_bulk']($scope.case_array);
            };
            $scope.SignUpload = function () {
                $scope.$parent['SignUpload']($scope.case_array);
            };
...
// Tooltip Directive starts here

dmCommonApi.directive('toolTip', toolTipDir);

toolTipDir.$inject = ['$parse'];

function toolTipDir($parse) {
    return {
        restrict: 'A',
        transclude: true,
        templateUrl: 'commonFilters/tooltip.html',
        scope: {
            position: '@position',
            title: '@tooltipTitle',
            toolTipData: '=toolTip'
        ...
        link: function (scope, element, attrs) {
            //                scope.toolTipData = $parse(attrs.toolTip)(element.scope());
            //                console.log(scope.position);
            scope.position = scope.position || 'default';
            scope.title = scope.title || '';
            scope.toolTipDataType = '';
            var checkType = {}.toString;
            switch (checkType.call(scope.toolTipData)) {
                case "[object String]":
                    scope.toolTipDataType = 'string';
                    //                        scope.toolTipData==''?scope.isToolTipVisible = false:scope.isToolTipVisible = true;
                    break;
                case "[object Object]":
                    scope.toolTipDataType = 'object';
                    break;
                case "[object Array]":
                    scope.toolTipDataType = 'array';
                    break;
            }

            scope.getToolTipWidth = function () {
                var styleObj = {};
                var clientWidth = element[0].getBoundingClientRect().width;
                if (scope.toolTipDataType === 'string') {
                    styleObj.width = clientWidth;
                }
                return styleObj;
            };

            //                scope.setTooltipVisibility = function () {
            //                    switch (scope.toolTipDataType) {
            //                        case 'string':
            //                            if(scope.toolTipData.length==0){
            ...
            scope.adjustPosition = function () {
                ...
                var viewPortWidth = window.innerWidth || document.documentElement.clientWidth,
                    ...
                    left, clientRect, top, bottom,
                    offsetWidth = element[0].offsetWidth,
                    offsetHeight = element[0].clientHeight,
                    toolTipheight = document.getElementsByClassName('tooltipContainer')[0].clientHeight;
                if (typeof element[0].getBoundingClientRect === 'function') {
                    clientRect = element[0].getBoundingClientRect();
                    switch (scope.toolTipDataType) {
                        case 'string':
                            left = clientRect.left + window.pageXOffset; //+ clientRect.width;
                            //                                top = clientRect.top + window.pageYOffset - 2*clientRect.height-5;
                            bottom = viewPortHeight - clientRect.bottom + clientRect.height;
                            styleObj.bottom = bottom + 'px';
                            break;
                        default:
                            left = clientRect.left + window.pageXOffset + clientRect.width;
                            top = clientRect.bottom + window.pageYOffset - clientRect.height;
                            styleObj.top = top + 'px';
                            break;
                    }
                    ...
                    if (top + toolTipheight > viewPortHeight) {
                        top = top - toolTipheight - offsetHeight;
                    }
                }
                //                    styleObj.top = top + 'px';
        ...
        controller: toolTipDirCtrl
    };
}

toolTipDirCtrl.$inject = ['$scope'];

function toolTipDirCtrl($scope) {
    $scope.isToolTipVisible = false;
    $scope.getClassBasedOnPosition = function () {
        var classObj = {};
        switch ($scope.position) {
            case 'bottom-center':
                classObj['bottom-center'] = true;
                break;
            default:
        ...
        if ($scope.toolTipDataType == 'string') {
            ...
            classObj['stringTipContainer'] = true;
        }
        return classObj;
    };

    $scope.checkDataType = function (type, data) {
        var isType = false;
        switch (type) {
            case 'object':
...
dmCommonApi.directive('searchlist', ['$filter', '$parse', function ($filter, $parse) {
    return {
        restrict: "E",
        require: '?ngModel',
        scope: {
            ...
            scope.setViewValue = function (value) {
                if (scope.returnValueType && scope.returnValueType.toLowerCase() == 'value') {
                    if (!value && scope.searchListInputModel && scope.searchListInputModel.length > 0) {
                        ...
                        scope.$emit("listid", value[scope.itemValue]);
        ...
        controller: ['$scope', '$filter', function ($scope, $filter, $attr) {
            ...
            $scope.getHoverStyle = function (index) {
                ...
                $scope.searchlistItemHover[index] ? hoverStyle = {
                    background: '#' + $scope.themecolor,
                    color: '#ffffff'
                } : hoverStyle = {
                    background: '#ffffff',
...
// Search list disposition nicky starts here
dmCommonApi.directive("searchableMultiselect", function ($timeout) {
    return {
        templateUrl: 'commonFilters/searchableMultiselect.html',
        restrict: 'AE',
        scope: {
            displayAttr: '@',
            selectedItems: '=',
...
// Search list disposition nicky Ends here
...
dmCommonApi.directive('dmTimePicker', ['$filter', '$parse', function ($filter, $parse) {
    return {
        restrict: "E",
        require: '?ngModel',
        scope: {
        ...
        link: function (scope, element, attrs, ngModelCtrl) {
            ...
            if (attrs.hasOwnProperty('ngDisabled')) {
                scope.pickerDisabled = $parse(attrs.ngDisabled)(element.scope());
            }

            var limit = false;
            if (attrs.hasOwnProperty('limit')) {
                limit = $parse(attrs.limit)(element.scope());
                //                    limit = true;
            }
            ...
            scope.minMins = 0;
            if (limit) {
                scope.minHour = new Date().getHours();
            }
            for (var i = scope.minHour; i < 24; i++) {
                ...
                if (itemType == '[object Object]') {
                    if (item.hasOwnProperty('$modelValue')) {
                        ...
                        if (item['$error']['isError'] && item['$error']['isVisible']) {
                            ...
                            console.log(" returnValue.errorMessages" + returnValue.errorMessages);
                        }
                    }
                }
            });
...
dmCommonApi.directive('validate', ['formValidityPrvdr', '$parse', '$filter', '$rootScope', function (formValidityPrvdr, $parse, $filter, $rootScope) {
    return {
        ...
        link: function (scope, element, attrs, ngModelCtrl) {
            ...
            var validationType = validationObj['textbox_type'] || dataType;
                ...
                switch (dataType) {
                    case 'text':
                        switch (validationType) {
                            ...
                            default:
                                if (!value) {
                                    errorMsg = parameterName + ' cannot be empty';
                                    returnValue = false;
                                }
                                break;
                        }
                        ...
                        switch (validationType) {
                            case 'datetime':
                                ...
                                if (valueType == '[object Object]') {
                                    ...
                                    if (value['date'] && value['time'] == "00:00") {
                                        errorMsg = 'Time' + ' must be selected';
                                        returnValue = false;
                                    }
                                }
                                break;
                            ...
                            switch (validationType) {
                                case 'date':
                                    ...
                                    if (!compareResult.isValid) {
                                        ...
                                        errorMsg = 'Date of birth should not be greater than ' + compareResult.message + '(should be 18 years or above)';
                                        returnValue = false;
                                    }
                                    break;
                                case 'datetime':
            ...
            var chkForMaxLength = function (dataType, validationType, val, oldValue, maxlen, mandatory) {
                ...
                if (maxlen && (mandatory || (!mandatory && valLength > 0))) {
                    switch (dataType) {
                        ...
                        case 'textarea':
                            if (valLength && valLength > maxlen) {
                                errorMsg = parameterName + ' length should be less than ' + maxlen;
                                returnValue = false;
                            }
                            break;
                            break;
                        ...
                        case 'text':
                            switch (validationType) {
                                ...
                                case 'alphanum':
                                    if (isBlurEvent && newValue && !newValue.match(regex)) {
                                        returnValue = false;
                                        setError(true, parameterName + ' should be alpha numeric in uppercase');
                                    }
                                    break;
                                case 'firstalpha':
                                    if (newValue && !newValue.match(regex)) {
                                        returnValue = false;
                                        setError(true, parameterName + ' should start with alphabet in uppercase');
                                        //  setError(true, parameterName + ' should not contain white space characters');
                                    }
                                    break;
                                ...
                                case 'onlyspace':
                                    if (newValue && !newValue.match(regex)) {
                                        returnValue = false;
                                        setError(true, 'In ' + parameterName + ' field space and special characters are not allowed except (/ and -)');
                                    }
                                    break;
                                ...
                                case 'onlyaadhar':
                                    if (newValue && !newValue.match(regex)) {
                                        returnValue = false;
                                        setError(true, parameterName + ' should  contain numbers only');
                                    }
                                    break;
                                case 'maxdep':
                                    if (newValue && !newValue.match(regex)) {
                                        // setPreviousValue(oldValue);
                                        returnValue = false;
                                        setError(true, 'In ' + parameterName + ' minimum value could be 0 and maximum value could be 20');
                                    }
                                    break;
                                case 'pincode':
                                    if (newValue && !newValue.match(regex)) {
                                        // setPreviousValue(oldValue);
                                        returnValue = false;
                                        setError(true, '' + parameterName + ' should be 6 digits long and only numbers are allowed');
                                    }
                                    break;
                                case 'number':
                                case 'n':
            ...
            var validateInput = function (newValue, oldValue, scope, isBlurEvent) {
                ...
                try {
                    if (chkForRegex(newValue, oldValue, scope, isBlurEvent)) {
                        ...
                        if (validationObj['parent_form_action']) {
...
dmCommonApi.directive('dmSearchList', ['$parse', '$http', '$compile', '$filter', '$rootScope', function ($parse, $http, $compile, $filter, $rootScope) {
    return {
        ...
        link: function (scope, element, attrs, ngModalCtrl) {
            ...
            //                scope.itemList = $filter('orderObjectBy')('value_sequence');

            //                scope.itemList.unshift(addDefaultValue('Select'));
            function addDefaultValue(val) {
                var listObj = scope.itemList[0];
                var newObj = Object.assign({}, listObj);
                angular.forEach(newObj, function (value, objKey) {
                    newObj[objKey] = val;
                });
                return newObj;
            };
            ...
            scope.keys.push({
                ...
                action: function (event) {
                    scope.itemClick(scope.itemList[scope.focusIndex]);
                    scope.listObj.listShown = false;
            ...
            scope.keys.push({
                code: 40,
                action: function (event) {
                    if (scope.focusIndex < scope.itemList.length) {
                        scope.focusIndex++;
                    }
                }
            });
            ...
            //                    var code = e.keyCode;
            //                    scope.focusIndex = scope.focusIndex==null?0:scope.focusIndex;
...
dmCommonApi.filter('byDate', function () {
    return function (collection, range, key) {
        var filteredArr = [];
        for (var c = 0; c < collection.length; c++) {
            var cDate = collection[c][key].parseDate('todateobj');
            if (cDate <= range.to && cDate >= range.from) {
                filteredArr.push(collection[c]);
            }
        }
        return filteredArr;
    };
...
dmCommonApi.filter('doubleDigit', function () {
    return function (value) {
...
dmCommonApi.filter('ellipsis', function () {
    return function (input, length) {
        length = parseInt(length) || 25;
        var subString = input;
        if (input.length > length) {
            subString = input.substr(0, length);
            subString = subString + '...';
        }
        return subString;
    };
